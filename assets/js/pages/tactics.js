(() => {
  const tabset = document.querySelector('[data-tactics-tabset="section"]');
  if (!tabset) return;

  const tabs = Array.from(tabset.querySelectorAll(":scope > [data-tactics-tab]"));
  const panels = Array.from(document.querySelectorAll('[data-tactics-panel="section"]'));
  ...
  const activateTab = (tab, options = {}) => {
    ...
    const targetId = tab.dataset.tacticsTab;
  ...
  const activateFromHash = (options = {}) => {
    ...
    const requestedTab = tabs.find((tab) => tab.dataset.tacticsTab === requestedId);
