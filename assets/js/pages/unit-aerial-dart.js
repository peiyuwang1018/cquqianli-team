(() => {
  ...
  const sectionById = new Map(sections.map((section) => [section.dataset.airDartSection, section]));

  const setActive = (requestedId) => {
    const id = sectionById.has(requestedId) ? requestedId : "aerial";
    links.forEach((link) => {
  ...
  links.forEach((link, index) => {
    link.addEventListener("click", (event) => {
      ...
      const id = link.dataset.airDartNav;
  ...
  const syncActiveToScroll = () => {
    ...
    setActive(current.dataset.airDartSection);
  };
